		...
		if (tws.upgradeReq.url === '/test') {
			...
			tws.on('message', function (m, raw) {
				...
				if (m.event === 'new-user') {
					...
					for (const pastAnswer of tws.crew.recentAnswers) {
				...
				if (m.event === 'new-game') {
					dbcs.qsets.findOne({ _id: m.qsetID }, function (err, qset) {
						...
						for (const question of tws.game.questions) {
							for (const answer of question.answers) if (!answers.includes(answer)) answers.push(answer);
							for (const answer of question.incorrectAnswers) if (!answers.includes(answer)) answers.push(answer);
						}
