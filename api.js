/* global generateID:true dbcs:true*/
...
module.exports = function(req, res, post) {
	if (req.url.pathname === '/new-qset') {
		...
		if (post.name.length > 144) {
			return res.writeHead(400) || res.end('Set name length must not be greater than 144 characters.');
		}
		let uquestions;
		try {
		...
		for (let i = 0; i < uquestions.length; i++) {
			...
			if (!q.text || !q.answer || !q.incorrectAnswers) {
				res.writeHead(400) || res.end(`Question ${i} is malformed.`);
			} else if (q.text.length > 144) {
				res.writeHead(400) || res.end(`Question ${i} is too long.`);
			} else if (q.answer.length > 64) {
				res.writeHead(400) || res.end(`The correct answer of question ${i} is too long.`);
			}
			for (let j = 0; j < q.incorrectAnswers.length; j++) {
				if (typeof q.incorrectAnswers[j] !== 'string') {
					res.writeHead(400) || res.end(`Incorrect answer ${j} of question ${i} is malformed.`);
				} else if (q.incorrectAnswers[j].length > 64) {
					res.writeHead(400) || res.end(`Incorrect answer ${j} of question ${i} is too long.`);
				}
			}
			questions.push({
				text: q.text,
